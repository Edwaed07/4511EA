            ...
            <div class="col-md-8">
                <div class="card">
                    <div class="card-header bg-warning text-dark">
                        <h5 class="mb-0"><i class="bi bi-dash-circle"></i> 水果出库</h5>
                    </div>
                    <div class="card-body">
                        <!-- 错误提示 -->
                        ...
                        <form action="${pageContext.request.contextPath}/warehouse/check-out" method="post">
                            <div class="mb-3">
                                <label for="inventoryId" class="form-label">选择库存项</label>
                                <select class="form-select" id="inventoryId" name="inventoryId" required>
                                    <option value="">-- 请选择要出库的水果 --</option>
                                    <c:forEach var="inventory" items="${inventoryList}">
                                        <option value="${inventory.inventoryId}">
                                            ${inventory.fruitName} - 当前库存: ${inventory.quantity} (位置: ${inventory.warehouseLocation})
                                        </option>
                                    </c:forEach>
                                </select>
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <div class="form-text">请确保出库数量不超过当前库存</div>
                            </div>
                            
                            <div class="mb-3">
                                <label for="destinationLocation" class="form-label">目的地</label>
                                <input type="text" class="form-control" id="destinationLocation" name="destinationLocation" required>
                                <div class="form-text">指明水果的运送目的地，例如：面包店名称或城市</div>
                            </div>
