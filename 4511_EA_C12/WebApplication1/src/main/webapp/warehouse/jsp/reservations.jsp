            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header bg-primary text-white d-flex justify-content-between align-items-center">
                        <h5 class="mb-0"><i class="bi bi-calendar-check"></i> 预订管理</h5>
                    </div>
                    <div class="card-body">
                        <!-- 过滤器 -->
                        <div class="row mb-4">
                            <div class="col-md-6">
                                <form action="${pageContext.request.contextPath}/warehouse/reservations" method="get" class="d-flex">
                                    <select class="form-select" name="status">
                                        ...
                                        <option value="completed" ${statusFilter eq 'completed' ? 'selected' : ''}>已完成</option>
                                    </select>
                                ...
                                <tbody>
                                    <c:choose>
                                        <c:when test="${empty reservations}">
                                            <tr>
                                                <td colspan="9" class="text-center">暂无预订记录</td>
                                            </tr>
                                        </c:when>
                                        <c:otherwise>
                                            <c:forEach var="reservation" items="${reservations}">
                                                <tr>
                                                    ...
                                                    <td>${reservation.fruitName}</td>
                                                    <td>${reservation.quantity}</td>
                                                    <td>${reservation.bakeryName}</td>
                                                    <td><fmt:formatDate value="${reservation.expectedDeliveryDate}" pattern="yyyy-MM-dd" /></td>
                                                    <td>
                                                        <c:choose>
                                                    ...
                                                    <td>
                                                        <div class="btn-group btn-group-sm">
                                                            <c:if test="${reservation.status eq 'pending'}">
                                                                <a href="${pageContext.request.contextPath}/warehouse/reservation-approval?id=${reservation.reservationId}" class="btn btn-outline-primary">
                                                                    <i class="bi bi-check-square"></i> 审批
                                                                </a>
                                                            </c:if>
                                                            <c:if test="${reservation.status eq 'approved' && empty reservation.deliveryId}">
                                                                <a href="${pageContext.request.contextPath}/warehouse/create-delivery?reservationId=${reservation.reservationId}" class="btn btn-outline-success">
                                                                    <i class="bi bi-truck"></i> 安排配送
                                                                </a>
                                                            </c:if>
                                                        </div>
                                                    </td>
